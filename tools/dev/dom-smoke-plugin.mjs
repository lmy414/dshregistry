...
assert(!!badge && badge.classList.contains('vouched'), '社区认可徽章')
assert(!!badge.title && badge.title.includes('stars 1043'), `信任徽章 stateReasons 悬浮依据 ("${badge.title}")`)
assert(doc.querySelector('.plugin-desc')?.textContent.includes('architecture-aware'), '描述')
assert(doc.querySelector('.plugin-stars')?.textContent === '1043', 'stars')
...
assert(!!warning && warning.classList.contains('vouched') && warning.textContent.includes('社区认可'), '社区认可琥珀警示条')
...
assert(ext2.textContent.includes('DSH Hub 状态') && ext2.textContent.includes('发现中'), 'DSH Hub 卡标题+状态')
