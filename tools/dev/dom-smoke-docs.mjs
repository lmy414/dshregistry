...
console.log('== 文档页:站长精选说明 + 信任三态 + 免责 + 合规 ==')
// 站长精选说明:关于本站 → 信任三态之前(维护活跃/生态/个人体验 + 不代表安全)
const featuredNote = [...dd.querySelectorAll('#about .docs-subsection')].map((s) => s.querySelector('.sub-heading')?.textContent).indexOf('站长精选')
assert(featuredNote !== -1, '关于本站含"站长精选"子区块')
const featuredSection = dd.querySelectorAll('#about .docs-subsection')[featuredNote]
assert(featuredSection?.textContent.includes('维护活跃、生态以及个人体验'), '站长精选说明含选择依据')
assert(featuredSection?.textContent.includes('不代表一定能运行') && featuredSection?.textContent.includes('风险自负'), '站长精选说明含风险免责')
